...
export async function GET(req: NextRequest) {
    try {
        ...
        if (!fitnessGoals.length) {
            return NextResponse.json({ error: "No fitness goals found" }, { status: 404 });
        }
...
export async function POST(req: NextRequest) {
    try {
        ...
        const { goal_type, target_value, current_progress, start_date, end_date } = await req.json();

        if (!goal_type || !target_value || !current_progress || !start_date || !end_date) {
            return NextResponse.json({ error: "All fields are required" }, { status: 400 });
        }

        await fitnessGoalsRepo.createFitnessGoal(userId, goal_type, target_value, current_progress, start_date, end_date);

        return NextResponse.json({ message: "Fitness goal created successfully" }, { status: 201 });
    } catch (error: any) {
        return NextResponse.json({ error: error.message }, { status: 500 });
    }
}

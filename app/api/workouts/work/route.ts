...
export async function GET(req: Request) {
    try {
        ...
        if (!workouts.length) {
            return NextResponse.json({ error: "No Data To Show in Workouts By User" }, { status: 404 });
        }
...
export async function POST(req: Request) {
    try {
        const id:any = req.headers.get("id");
        const { exercise_type, duration, calories_burned, workout_date } = await req.json();
...
export async function PATCH(req: Request) {
    try {
        const id:any = req.headers.get("id");
        const { workout_id, exercise_type, duration, calories_burned, workout_date } = await req.json();

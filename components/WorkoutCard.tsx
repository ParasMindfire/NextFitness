  ...
  return (
    <div className='bg-primary rounded-2xl shadow-xl p-8 w-full text-white transition-transform transform hover:scale-105 hover:shadow-2xl'>
      ...
      <div className='space-y-3 text-white text-sm'>
        <p>
          <span className='font-semibold text-white'>{DURATION_LABEL}</span>{' '}
          {workout.duration} mins
        </p>
        <p>
          <span className='font-semibold text-white'>{CALORIES_LABEL}</span>{' '}
          {workout.calories_burned} kcal
        </p>
        <p>
          <span className='font-semibold text-white'>{DATE_LABEL}</span>{' '}

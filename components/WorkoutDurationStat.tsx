...
export const WorkoutDurationStats = () => {
  ...
  useEffect(() => {
    if (workouts.length > 0) {
      ...
      const monthly = workouts.filter(
        (workout) =>
          new Date(workout.workout_date) >=
          new Date(today.setMonth(today.getMonth() - 1))
      );
      const yearly = workouts.filter(
        (workout) =>
          new Date(workout.workout_date) >=
      ...
      setWeeklyData(
        weekly.map((w) => ({ date: w.workout_date, duration: w.duration }))
      );
      setMonthlyData(
        monthly.map((w) => ({ date: w.workout_date, duration: w.duration }))
      );
      setYearlyData(
        yearly.map((w) => ({ date: w.workout_date, duration: w.duration }))
      );
    }
  }, [workouts]);
      ...
      {(() => {
        ...
        return (
          <>
            ...
            <div className='mt-8'>
              <h3 className='text-lg font-semibold text-center text-blue-600 md:text-xl'>
                {WEEKLY_PROGRESS}
              </h3>
            ...
            <div className='mt-8'>
              <h3 className='text-lg font-semibold text-center text-green-600 md:text-xl'>
                {MONTHLY_PROGRESS}
              </h3>
              <p className='text-sm text-center text-gray-500'>
            ...
            <div className='mt-8'>
              <h3 className='text-lg font-semibold text-center text-yellow-600 md:text-xl'>
                {YEARLY_PROGRESS}
              </h3>
              <p className='text-sm text-center text-gray-500'>

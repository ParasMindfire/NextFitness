...
export const getAllWorkouts = async (): Promise<any> => {
    const [workouts]: any = await sequelize.query("select email,name , exercise_type ,duration,calories_burned,workout_id from users inner join workouts on users.user_id=workouts.user_id;");
    return workouts;
};

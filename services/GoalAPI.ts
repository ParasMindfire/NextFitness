...
export const getAllFitnessGoals = async (token: string) => {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch fitness goals (${response.status} - ${response.statusText})`);
    }
...
export const createFitnessGoal = async (token: string, goal: any) => {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Failed to create fitness goal (${response.status} - ${response.statusText})`);
    }

    return await response.json();
  } catch (error) {
    console.error("Error creating fitness goal:", error);
    throw new Error("Unable to create fitness goal. Please check your input and try again.");
  }
};
...
export const updateFitnessGoal = async (token: string, id: number | null, goal: any) => {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Failed to update fitness goal (${response.status} - ${response.statusText})`);
    }
...
export const deleteFitnessGoal = async (token: string, id: number) => {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Failed to delete fitness goal (${response.status} - ${response.statusText})`);
    }
...
export const getSingleFitnessGoal = async (token: string, id: number) => {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch single fitness goal (${response.status} - ${response.statusText})`);
    }

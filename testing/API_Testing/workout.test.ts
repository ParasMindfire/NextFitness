...
// import { GET as getWorkoutsByUser, PATCH as updateWorkout, DELETE as deleteWorkout, POST as createWorkout } from '../../app/api/workouts/work/route'; // Adjust the path as necessary
...
//       const res = await getAllWorkouts(req as any);
...
//       vi.spyOn(workoutRepo, 'getWorkoutsByUser').mockResolvedValue(mockWorkouts);
...
//       expect(res.status).toBe(404);
//       expect(responseJson).toEqual({ error: "No Data To Show in Workouts By User" });
//     });
//   });
...
//       vi.spyOn(workoutRepo, 'findWorkout').mockResolvedValue([{ workout_id: 17 }]);
...
//       expect(responseJson).toEqual({ error: "You have already added this exercise today" });
//     });
//   });
